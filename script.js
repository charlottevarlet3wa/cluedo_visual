...
function selectCharacter(characterKey) {
  ...
  gameState.humanPlayerIndex = gameState.players.findIndex(
...
function executeCommand(command) {
  ...
  const sheetRegex =
    /^sheet\.(suspects|weapons|rooms)\.([a-zA-Z]+)\.([a-zA-Z]+)\.(text|backgroundColor)\s*=\s*['"](.+)['"]\s*;?$/;
  ...
  if (sheetMatch) {
    ...
    if (!sheet[category][row][column]) {
      displayError(
        `Invalid name "${column}" for row "${row}" in category "${category}".`
      );
      return;
    }
  ...
  const gameCommandRegex =
    /^game\.characters\.([a-zA-Z]+)\.([a-zA-Z]+)\(([^)]*)\)\s*;?$/;
  ...
  if (gameMatch) {
    ...
    if (typeof character[methodName] !== "function") {
      displayError(
        `Invalid method "${methodName}" for the character "${characterName}".`
      );
      return;
    }
    ...
    switch (methodName) {
      ...
      case "suggest":
        ...
        if (args.length === 3) {
          ...
          displayError(
            "The suggest command requires three parameters: character, weapon, and room."
          );
        }
        break;
      case "accuse":
        ...
        if (args.length === 3) {
          ...
          displayError(
            "The accuse command requires three parameters: character, weapon, and room."
          );
        }
        break;
      case "skip":
...
commandInput.addEventListener("keydown", function (event) {
  ...
  if (event.key.match(/[a-zA-Z0-9]/)) {
    ...
    if (args.length > 0) {
      ...
      const matchingOptions = suggestions.filter((option) =>
        option.startsWith(lastArgPart)
      );
      ...
      if (matchingOptions.length === 1) {
        ...
        if (argParts.length === 1) {
          ...
          args[args.length - 1] =
            argParts.slice(0, -1).join(".") + "." + matchingOptions[0];
        }
...
function saveGame() {
  ...
  characters.forEach((char) => {
    const pos = {
      x: char.getAttribute("x"),
      y: char.getAttribute("y"),
      room: char.getAttribute("room"),
    };
    savedPos.push(pos);
  });
  const saveData = {
    gameState: gameState, // Save the entire game state, including the cards
    savedSheet: sheet,
    savedSheetTables: sheetTables,
    savedLog: log.innerHTML,
    // savedPositions: characters.map((character) => ({
    //   x: character.getAttribute("x"),
    //   y: character.getAttribute("y"),
    //   room: character.getAttribute("room"),
    // })),
...
function resumeGame() {
  ...
  // Restore gameState (including cards)
  ...
  // Restore player cards (this is key to ensure the cards are correctly assigned)
  gameState.cards = savedGameState.cards; // Restoring the cards array
  ...
  // Restore the player cards visually
  // Resto're the placards :'ly
  // Restore the placardsly
  document.querySelector("#my-cards p").textContent =
    "My cards: " + gameState.cards[gameState.humanPlayerIndex];
  ...
  // Continue from the saved game state
...
const sheet2 = {
  mustard: {
    mustard: { text: "", backgroundColor: "" },
    green: { text: "", backgroundColor: "" },
    peacock: { text: "", backgroundColor: "" },
    scarlet: { text: "", backgroundColor: "" },
    white: { text: "", backgroundColor: "" },
  },
  green: {
    mustard: { text: "", backgroundColor: "" },
    green: { text: "", backgroundColor: "" },
...
function setupGame() {
  ...
  characters.forEach((character, index) => {
    const closestDoor = findClosestDoor(
      character.getAttribute("x"),
      character.getAttribute("y")
    );
...
function distributeCards() {
  ...
  const remainingCards = shuffleDeck([
    ...gameState.deck.characters,
    ...gameState.deck.weapons,
    ...gameState.deck.rooms,
  ]);
...
function nextPlayer() {
  if (!gameState.isStarted) {
    ...
    gameState.interrogatingCharacterIndex =
      (gameState.interrogatingCharacterIndex + 1) % gameState.players.length;
  }
...
function aiTurn() {
  ...
  switch (gameState.currentStep) {
    ...
    case "aiMove":
      setTimeout(() => {
        const isInRoom = aiMoveCharacter(
          characters[gameState.interrogatingCharacterIndex]
        );
...
function updateRoomStayCount(aiIndex) {
  ...
  if (currentRoom) {
    ...
    if (gameState.aiRoomStayCount[aiIndex][currentRoom] % 3 === 0) {
      ...
      displayMessage(
        `${gameState.players[aiIndex]} has stayed in ${currentRoom} for 3 turns, changing objective to ${newObjective.room}.`
      );
    }
  }
}
...
function getMostVisitedRooms(aiIndex) {
  ...
  const sortedRooms = Object.keys(stayCounts).sort(
    (a, b) => stayCounts[b] - stayCounts[a]
  );
  ...
  const mostVisitedRooms = sortedRooms.filter(
    (room) => stayCounts[room] >= thresholdCount
  );
...
function findNewObjective(characterX, characterY, availableRooms) {
  const availableDoors = doors.filter((door) =>
    availableRooms.includes(door.room)
  );
  ...
  availableDoors.forEach((door) => {
    const distance =
      Math.abs(door.x - characterX) + Math.abs(door.y - characterY);
...
function findClosestAccessibleSquare(accessibleSquares, targetDoor) {
  ...
  const currentRoom =
    characters[gameState.interrogatingCharacterIndex].getAttribute("room");
...
function aiMoveCharacter(character) {
  ...
  const closestSquare = findClosestAccessibleSquare(
    accessibleSquares,
    doorObjective
  );
  const characterName =
    gameState.players[gameState.interrogatingCharacterIndex];
...
function aiMakeSuggestion() {
  ...
  if (!currentRoom) {
    displayMessage(
      `${gameState.players[aiIndex]} cannot make a suggestion as they are not in a room.`
    );
    return;
  }
...
function aiCheckSuggestion(suggestion) {
  ...
  for (let i = 0; i < playerCount; i++) {
    ...
    if (commonCards.length == 0) {
      ...
      displayMessage(
        `✅ ${interrogatedName}.${
          interrogated === gameState.humanPlayerIndex
            ? " You show the card : " + shownCard
            : ""
        }`
      );
      return;
...
function rollDice() {
  ...
  displayMessage(
    `${selectedName} rolls the dice: ${dice}.${
      gameState.interrogatingCharacterIndex === gameState.humanPlayerIndex
        ? " Move to a square."
        : ""
    }`
  );
...
function move(x, y, resumeIndex = null) {
  ...
  if (!targetSquare.classList.contains("highlight") && resumeIndex == null) {
    displayError(
      `Vous ne pouvez pas vous déplacer sur la case (${x}, ${y}) car elle n'est pas accessible.`
    );
    return false;
  }
  ...
  const characterElement = document.getElementById(
    gameState.playersId[gameState.humanPlayerIndex]
  );
...
function makeSuggestion(character, weapon, room) {
  ...
  const currentRoom = roomAttributeToName(
    characters[playerIndex].getAttribute("room")
  );
  ...
  if (currentRoom.toLowerCase() !== room.toLowerCase()) {
    displayError(
      `You can only make a suggestion for the room you are currently in: ${roomAttributeToName(
        currentRoom
      )}.`
    );
    return false;
...
function checkSuggestion() {
  ...
  for (let i = 0; i < playerCount; i++) {
    ...
    if (commonCards.length == 0) {
      ...
      displayMessage(
        `✅ ${interrogatedName} shows you the card: ${shownCard}.`
      );
      return;
    }
...
function makeAccusation(character, weapon, room) {
  ...
  if (currentRoom.toLowerCase() !== room.toLowerCase()) {
    displayError(
      `You can only make an accusation for the room you are currently in: ${roomAttributeToName(
        currentRoom
      )}.`
    );
    return;
  ...
  if (
    ...
    displayMessage(
      `Well done! It was ${solution.character} with the ${solution.weapon} in the ${solution.room}.`
    );
  } else {
    displayMessage(
      `Your accusation: ${gameState.accusation} is wrong.  It was ${solution.character} with the ${solution.weapon} in the ${solution.room}.`
    );
